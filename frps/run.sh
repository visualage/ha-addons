...
if [[ ${TYPE} == "http" ]]; then
    ...
    /frpc http -s ${HOST}:${PORT} --sd ${HTTP_SUBDOMAIN_HOST} -i ${SERVER_LOCAL} -l ${PORT_LOCAL} -t ${TOKEN} -n ha_${HTTP_SUBDOMAIN_HOST} --locations / --tls_enable
...
if [[ ${TYPE} == "tcp" ]]; then
    ...
    /frpc tcp -s ${HOST}:${PORT} -i ${SERVER_LOCAL} -l ${PORT_LOCAL} -t ${TOKEN} -r ${TCP_PORT_REMOTE} -n ha_${HTTP_SUBDOMAIN_HOST} --tls_enable
